...
public class ChartResponse {
	...
	public static ChartResponse createHistoryResponse(KlineRestApiResponse response, int scale) {
		...
		BigDecimal open = response.getOpen().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal high = response.getHigh().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal low = response.getLow().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal close = response.getClose().setScale(scale, RoundingMode.HALF_UP);
	...
	public static ChartResponse createUpdateResponse(KlineData data, int scale) {
		...
		BigDecimal open = details.getOpen().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal high = details.getHigh().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal low = details.getLow().setScale(scale, RoundingMode.HALF_UP);
		BigDecimal close = details.getClose().setScale(scale, RoundingMode.HALF_UP);
